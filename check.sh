#!/bin/bash
# URLs for Jagoancoin explorers. Main and backup one.
URL=( 'https://explorer.Jagoancoin.org/' 'https://block.Jagoancoin.org/' )
URL_ID=0
...
#JAGOANCOIN_CLI    -> Path to the Jagoancoin-cli
#CONFIG_DIR/HOME  -> Path to "$HOME/.jagoancoincore/"
...
if [[ -z $JAGOANCOIN_CLI ]]; then
  JAGOANCOIN_CLI=$(which Jagoancoin-cli)
fi
...
if [[ -z $CONFIG_DIR ]]; then
  ...
  CONFIG_DIR="$HOME/.jagoancoincore/"
fi
...
function ReadCli () {
  ...
  # Jagoancoin-cli did not return after 300s. kill the PID and exit with -1.
  kill -9 $PID
  echo -1
}
...
function CheckPoSe () {
  ...
  # Check if we should restart Jagoancoind according to the PoSe score.
  if (( POSE_SCORE > 0 )); then
    if (( POSE_SCORE > PREV_SCORE )); then
      ...
      # Do not check node height after killing Jagoancoind it is sure to be stuck.
      exit
  ...
  if [[ $POSE_SCORE -eq $PREV_SCORE || $PREV_SCORE -eq -1 ]]; then
    ...
    if [[ $((NETWORK_HEIGHT - LOCAL_HEIGHT)) -gt 3 || $NETWORK_HEIGHT == -1 ]]; then
      if (( LOCAL_HEIGHT > PREV_HEIGHT )); then
        ...
        # This meand simple Jagoancoind kill will not help and we need to
...
function BootstrapChain () {
  ...
  echo "$(date -u)  Download and prepare jgc-bootstrap."
  rm -rf /tmp/bootstrap 2>/dev/null
  mkdir -p /tmp/bootstrap 2>/dev/null
  curl -L "$BOOTSTRAP_TAR" | tar xJ -C /tmp/bootstrap/
  
  # Stop serivce and kill Jagoancoind.
  echo "$(date -u)  Kill Jagoancoind."
  sudo systemctl stop Jagoancoin
  killall -9 Jagoancoind 2>/dev/null
  ...
  sudo systemctl start Jagoancoin
}

# This should force unstuck the local node.
function ReconsiderBlock () {
  # If Jagoancoin-cli is responsive and it is stuck in the different place than before.
  ...
  # Jagoancoin-cli is/was unresponsive in at least 1 step
  return 1
}
